  ...
  useEffect(() => {
    async function fetchCategories() {
      try {
        ...
        // Find the "Beauty & Health" category
        const beautyCategory = categoriesArray.find(
          (cat) => cat.slug === "beauty-health"
        );

        if (beautyCategory) {
          setSubCategories(beautyCategory.categories || []);
        }
      } catch (err) {
          ...
          {subCategories.map((category) => (
            ...
            >
              ...
              <div className="absolute inset-0 flex items-center justify-center p-4">
                <h2 className="text-white text-3xl sm:text-4xl font-extrabold text-center drop-shadow-lg bebas-font">
                  {category.name}
                </h2>
              </div>
            </Link>
