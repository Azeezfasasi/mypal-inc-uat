...
export default function EventTicketCategory() {
  ...
  useEffect(() => {
    async function fetchCategories() {
      try {
        ...
        const eventCategory = categoriesArray.find(
          (cat) => cat.slug === "event-ticketing"
        );

        if (eventCategory) {
          setSubCategories(eventCategory.categories || []);
        }
      } catch (err) {
