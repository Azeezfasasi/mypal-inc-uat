                ...
                <div className="flex flex-col lg:flex-row gap-8">
                    ...
                    <div className="lg:w-2/3">
                        ...
                        <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4 mb-8">
                            <button onClick={() => setShowDownloadModal(true)} className="w-full sm:w-auto px-4 md:px-8 py-3 md:py-5 rounded-[10px] bg-[#DB3A06] text-white font-semibold shadow-md hover:bg-orange-700 transition-colors duration-200 gap-2.5 cursor-pointer text-[14px] md:text-[16px] bebas-font">
                        ...
                        <div className="bg-white rounded-[10px] p-6 md:p-8 shadow-xl border border-solid border-gray-300 mb-8">
                            <h3 className="text-xl md:text-2xl font-semibold text-[#000000] mb-6 bebas-sub-title-font">
                                Service Offered
                            </h3>
                                ...
                                {apiServices && apiServices.length > 0 ? (
                                    apiServices.map((service, index) => (
                                        <div key={index} className="flex items-center space-x-2 text-gray-700">
                                            ...
                                            <span className="text-[#000000] text-[14px] md:text-base font-normal bebas-font">{service}</span>
                                        </div>
                                    ))
                                ) : (
                                    <p className="text-gray-500 bebas-font">No services offered information available.</p>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="lg:w-1/3 hidden md:block">
                        <div className="bg-white rounded-[10px] p-6 md:p-8 border border-solid border-gray-300">
                            ...
                            {/* Location and Hours */}
                            <h4 className="text-[15px] font-semibold text-gray-800 mb-4 bebas-font">Business Hours</h4>
                            <div className="space-y-3">
                                {operatingDays.map((item, index) => (
                                    <div key={index} className="flex justify-between text-[#000000] text-[14px] md:text-[15px] bebas-font">
                            ...
                            {/* Download Button */}
                            <div className="mt-8">
                                <button onClick={() => setShowDownloadModal(true)} className="w-full p-3 md:p-5 rounded-[10px] bg-[#DB3A06] text-[#FBEBE6] text-base font-semibold hover:bg-orange-700 transition-colors duration-200 flex flex-row gap-2.5 items-center justify-center cursor-pointer text-[14px] md:text-[16px] bebas-sub-title-font">
                                    {getDownloadButtonText(business?.category)}
                                </button>
                            </div>
                        </div>
